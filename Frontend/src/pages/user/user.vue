  ...
  <scroll-view scroll-y class="app-page-shell h-screen">
    <view class="px-28rpx pb-150rpx pt-38rpx">
      ...
      <UserProfileCard
        ...
        :subtitle="user.loggedIn ? maskedPhone : '登录后保存个人资料'"
        :avatar="user.avatar"
      ...
      <UserAssetSummary
        ...
        :streak-days="rewards.streakDays"
        :completed-tasks="completedTasks"
        @earnings="go('/pages/earnings/earnings')"
        @tasks="go('/pages/tasks/tasks')"
      />
...
import UserMenuList, {
  type UserMenuItem,
} from "../../components/user/UserMenuList.vue";
...
const menu = computed<UserMenuItem[]>(() => [
  ...
  { title: "我的收藏", icon: "star", route: "/pages/favorites/favorites" },
  {
    title: "通知消息",
    icon: "bell",
    route: "/pages/notifications/notifications",
...
const handleMenu = (item: UserMenuItem) => {
  if (!item.route)
    return uni.showToast({ title: item.message || "功能开发中", icon: "none" });
  if (item.route.includes("/tasks/") || item.route.includes("/earnings/"))
