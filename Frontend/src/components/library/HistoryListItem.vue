<template>
  <view class="flex py-22rpx" @click="$emit('play', item)">
    <view class="h-164rpx w-116rpx shrink-0 overflow-hidden rounded-14rpx bg-[#24252d]"><up-image width="100%" height="100%" mode="aspectFill" :src="item.image" /></view>
    <view class="ml-18rpx flex min-w-0 flex-1 flex-col">
      ...
      <text class="mt-10rpx block text-22rpx text-[#b8bac4]">看到第 {{ item.episode }} 集 · {{ positionText }}</text>
      <view class="mt-auto">
